...
class InsurancePremiumReportService {
  // GET API: Fetch available years (Staff Module)
    ...
    try {
      final response = await http.get(
        ...
        headers: {
          ...
          "id": "CRM $staffId", // Use staff_id for Staff Module
        },
      );
  ...
  // POST API: Fetch insurance premium report data (Staff Module)
  Future<InsurancePremiumReportModel> fetchInsurancePremiumReport({
    ...
    try {
      ...
      print("insurance premium report (Staff) ${response.body}");
      if (response.statusCode == 200) {
        final jsonData = json.decode(response.body);
