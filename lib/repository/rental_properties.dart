...
class Rental_PropertiesRepository {
  ...
  Future<Map<String, dynamic>> addRentals({
    ...
    if (responseData["statusCode"] == 200) {
      ...
      return rentalOwnerExists ??
          false; // Return rentalOwnerExists if not null, otherwise return false
    } else {
      Fluttertoast.showToast(msg: responseData["message"]);
  ...
  Future<Map<String, dynamic>> DeleteRentalOwner({required String? id}) async {
    // print('$apiUrl/$id');
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('token');
    String? id = prefs.getString('adminId');
