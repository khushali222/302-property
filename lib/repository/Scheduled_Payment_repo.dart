...
class Scheduled_Payment_repo{
  Future<List<Scheduled_Payment>> fetchScheduled_Payment() async {
    ...
    final response = await http.get(Uri.parse('${Api_url}/api/payment/upcomingpayment/$id'),
        headers: {
  ...
  Future<Map<String, dynamic>> DeleteScheduled_Payment({
    ...
    final String apiUrl = '${Api_url}/api/payment/payment/$pro_id';
    //print('$apiUrl/$id');
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('token');
  ...
  String? companyName =   prefs.getString('companyName');
    final http.Response response = await http.delete(
        Uri.parse('$apiUrl?company_name=$companyName'),
