...
class _MakePaymentState extends State<MakePayment> {
  ...
  // payment. Rather than telling the user "check the ledger yourself", read the
  // ledger and answer the question for them.
  final PaymentReconciliationService _reconciliation =
      PaymentReconciliationService();

  /// Payment ids present before we submit, so a payment that appears afterwards
  ...
  /// Guard against charging twice. If the previous attempt on this lease ended
  /// with an unknown outcome, check the ledger before sending anything.
  ///
  /// Returns true when the caller must ABORT — the earlier payment already
  /// posted. Returns false in every other case, including any failure to check,
  /// so this can never block a legitimate payment.
  Future<bool> _alreadyPaid(String tenantId) async {
    try {
      final posted = await _reconciliation.blockingDuplicate(
        leaseId: widget.leaseId,
        tenantId: tenantId,
      );
      if (posted == null) return false;

      final amount = posted.totalAmount?.toStringAsFixed(2) ?? '';
      ...
      logError('duplicate guard failed, allowing payment: $e');
      return false; // never block a payment because the check itself broke
    }
  }

  /// Shows the outcome of a failed sale. For an unknown outcome it first checks
  /// the ledger, so the dialog can state what actually happened.
  Future<void> _handleSaleFailure(
    ...
    switch (result.verdict) {
      case PaymentVerdict.completed:
        // It went through. Clear the key so a later payment gets a fresh one,
        // and drop any pending record — there is nothing left to resolve.
        _saleIdempotencyKey = null;
        _saleKeyScope = null;
      ...
      case PaymentVerdict.unknown:
        // Still can't tell. Remember it so the NEXT submit on this lease is
        // checked before it is allowed to charge again.
        await _reconciliation.rememberPending(PendingPaymentAttempt(
          leaseId: widget.leaseId,
          tenantId: tenantId,
          amount: amount,
  ...
  void initState() {
    ...
    // Payment settings will be fetched after tenant selection
  }
          ...
          for (final item in billing) {
            ...
            if ((checkAccount != null && checkAccount.isNotEmpty) ||
              ...
              list.add({
                ...
                'routing_number':
                    _extractVaultString(item['check_aba']) ?? '',
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // If we have a preset tenant ID from widget
          if (widget.tenantId.isNotEmpty) {
            // Find the tenant in the fetched list
            ...
            // Fetch necessary data for the preset tenant
            fetchChargesForSelectedTenant(selectedTenantId!);
            fetchcreditcard(selectedTenantId!);
            fetchPaymentSettings();
          }
          // If no preset tenant, handle based on number of tenants
      ...
      Fluttertoast.showToast(msg: 'Could not load tenants.');
    }
  }
  ...
  bool getEnableOverrideFee(String tenantId) {
    for (var tenant in tenants) {
      if (tenant['tenant_id'] == tenantId) {
        final raw = tenant['enableoverridefee'];
  ...
  // Update payment methods with availability status
  void _updatePaymentMethods() {
    _paymentMethods = [
      'Card', // Always show Card option
      'Check',
      'Cash',
          ...
          setState(() {
            ...
            if (enableOverrideFee &&
              ...
              surCharge = surchargeData['surcharge_percent_debit'] != null
                  ? num.tryParse(
                              surchargeData['surcharge_percent_debit'].toString())
                          ?.toDouble() ??
                      0.0
                  : 0.0;
            }
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
                                          ...
                                          onChanged: (value) async {
                                            ...
                                            setState(() {
                                              ...
                                              if (value == 'external_source') {
                                                ...
                                                fetchPaymentSettings(); // Fetch payment settings when tenant is selected
                                                _paymentMethods = [
                                                  'Card',
                                                  'Check',
                                                  'Cash',
                              ...
                              DropdownButtonHideUnderline(
                                child: DropdownButton2<String>(
                                  ...
                                  onChanged: (String? newValue) {
                                    ...
                                    if (cleanValue == 'Card' && !creditcard) {
                                      isAvailable = false;
                                    } else if (cleanValue == 'ACH' &&
                                        !achaccepted) {
                                      isAvailable = false;
                                    }

                                    // Only update selection if method is available
                                    if (isAvailable) {
                                      setState(() {
                                        _selectedPaymentMethod = cleanValue;
                                        AddFields();
                                      });
                                      surge_count();
                                      if (_selectedPaymentMethod == "Card") {
                                  ...
                                  return Column(
                                    children: [
                                      ...
                                      */
                                                    ...
                                                    if (cardType == 'CREDIT') {
                                                      isAccepted = creditcard;
                                                    } else if (cardType ==
                                                        'DEBIT') {
                                                      isAccepted = debitcard;
                                                    }
                                                          ...
                                                          isExpired == true
                                                              ...
                                                              : Checkbox(
                                                                  ...
                                                                  onChanged: (bool?
                                                                      value) async {
                                                                    if (!isAccepted) {
                                                                      // Show toast for disabled cards
                                                                      String
                                                                          cardType =
                                                                          item.binResult?.toUpperCase() ??
                                                                              '';
                                                                      Fluttertoast
                                                                          .showToast(
                                                                        msg:
            ...
            //cancel and apply payment button
            Padding(
              padding: EdgeInsets.only(
                  left: MediaQuery.of(context).size.width < 500 ? 18 : 35,
                  right: 16,
                              ...
                              if (_selectedPaymentMethod == null) {
                                ...
                                if (processor_id == "zzz") {
                                  ...
                                  await PaymentService()
                                    ...
                                    _settleSaleKey(e);
                                    // Spinner stays up while we ask the ledger
                              ...
                              if (_selectedPaymentMethod == null) {
                                ...
                                if (processor_id == "zzz") {
                                  ...
                                  await PaymentService()
                                    ...
                                    customerVaultId:
                                        (selectedcardindex != null &&
                                            ...
                                            ? (cardDetails[selectedcardindex!]
                                                ...
                                                "")
                                            : "",
                                    billingId: (selectedcardindex != null &&
                                            selectedcardindex! >= 0 &&
                                            selectedcardindex! <
                                                cardDetails.length)
                                        ? (cardDetails[selectedcardindex!]
                                    ...
                                    tenantId: selectedTenantId!,
                                    date: reverseFormatDate(_startDate.text.trim()),
                                    address1: (selectedcardindex != null &&
                                            selectedcardindex! >= 0 &&
                                            selectedcardindex! <
                                                cardDetails.length)
                                        ? (cardDetails[selectedcardindex!]
                                ...
                                String notificationTime =
                                    formatter.format(DateTime.now());
                                await _snapshotLedgerBeforeSale(selectedTenant != null ? selectedTenantId! : "");
                                _beginSale();
                                await PaymentService()
                                    .makePaymentfornormal(
                                  idempotencyKey: _saleIdempotencyKey,
