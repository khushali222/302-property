...
class _CustomReportBuilderState extends State<CustomReportBuilder> {
  ...
  SavedReport? get _dropdownValue {
    ...
    try {
      return _savedReports
          .firstWhere((r) => r.reportId == _selectedReport!.reportId);
    } catch (_) {
      return null;
    }
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_loading)
                          ...
                          // Table header: Create + Export buttons (same width)
                          Row(
                            children: [
                              Expanded(
                                flex: 1,
                                ...
                                child: Container(
                                  ...
                                  child: DropdownButtonHideUnderline(
                                    child: DropdownButton2<SavedReport>(
                                      ...
                                      onChanged: _savedReports.isEmpty
                                          ? null
                                          : (value) {
                                              setState(() {
  ...
  /// Format currency with $ first (e.g. $-8,073.60 for negative)
  ...
  Widget _buildExpandedContent(
    ...
    final historyDetailsList =
        historyDetailsRaw is List ? historyDetailsRaw : <dynamic>[];
      ...
      for (final e in historyDetailsList) {
        if (e is Map) {
          ...
          if (date.toString().isNotEmpty && val != null) {
            final fmt = val is num ? formatCurrency(val) : val.toString();
        ...
        return SingleChildScrollView(
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                // Card-based grid for property details (like design image)
                ...List.generate((entries.length / 2).ceil(), (i) {
                  final left = entries.length > i * 2 ? entries[i * 2] : null;
                  final right =
                      entries.length > i * 2 + 1 ? entries[i * 2 + 1] : null;
                  return Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Expanded(
                            child: left != null
                                ? _buildDetailCard(left.key, left.value)
                                : const SizedBox.shrink()),
                        const SizedBox(width: 10),
                        Expanded(
                            child: right != null
                                ? _buildDetailCard(right.key, right.value)
                                : const SizedBox.shrink()),
                      ],
                    ),
                  );
                }),
                // Historic Insured Values only (no History text); collapsible to save space
        ...
        children: [
          ...
          ...List.generate(_reportData.length, (index) {
            ...
            if (columns.length >= 2) {
              ...
              final v1 =
                  _formatCellValue(k1, row[k1], dateProvider.formatCurrentDate);
              final v2 =
                  _formatCellValue(k2, row[k2], dateProvider.formatCurrentDate);
