...
class _DailyTransactionsState extends State<DailyTransactions> {
  ...
  Future<List<DailyTransactionReport>> fetchDelinquentTenantsData(String fromDate, String toDate, {String? charge}) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? id = prefs.getString("adminId");
      String? token = prefs.getString('token');
      ...
      List<DailyTransactionReport> data = await DailyTrasactionReport().fetchDailyTransactions(id!, reverseFormatDate(fromDate), reverseFormatDate(toDate), chargetype: chargedata);
