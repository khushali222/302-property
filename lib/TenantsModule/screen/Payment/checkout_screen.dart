...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void onGooglePayResult(Map<String, dynamic> paymentResult) async {
    ...
    String paymentTokenString = jsonEncode(paymentResult);

    // ============================================
    // PRINT TOKEN FOR API DEVELOPER
    // ============================================
    const separator =
        "================================================================================";
    const dashSeparator =
        "--------------------------------------------------------------------------------";
    print("\n$separator");
    print("GOOGLE PAY TOKEN RECEIVED");
    print(separator);
    print("This is the token that needs to be sent to your API:");
    print(dashSeparator);
    // Print formatted JSON for better readability
    try {
      final encoder = JsonEncoder.withIndent('  ');
      print(encoder.convert(paymentResult));
    } catch (e) {
      print(paymentTokenString);
    }
    print(dashSeparator);
    print("Token as single-line JSON string (for API):");
    print(paymentTokenString);
    print("$separator\n");
  ...
  void onApplePayResult(Map<String, dynamic> paymentResult) async {
    ...
    const dashSeparator =
        "--------------------------------------------------------------------------------";
    print("\n$separator");
    print("APPLE PAY TOKEN RECEIVED");
    print(separator);
    print("This is the token that needs to be sent to your API:");
    print(dashSeparator);
    // Print formatted JSON for better readability
  ...
  Future<PaymentResponse> sendPaymentTokenToServer({
    ...
    // ============================================
    // PRINT API REQUEST DATA FOR API DEVELOPER
    // ============================================
    const separator =
        "================================================================================";
    const dashSeparator =
        "--------------------------------------------------------------------------------";
    print("\n$separator");
    print("API REQUEST DATA - ${wallet.toUpperCase()}");
    print(separator);
    print("Amount: \$$amount");
    print("Wallet Type: $wallet");
    print(dashSeparator);
    print(
        "Payment Token (paymentData) - This is what you need to send to your API:");
    print(dashSeparator);
    // Print formatted JSON for better readability
    try {
      final encoder = JsonEncoder.withIndent('  ');
      print(encoder.convert(paymentData));
    } catch (e) {
      print(tokenString);
    }
    print(dashSeparator);
    print("Complete API Request Body Structure:");
    print(dashSeparator);
    final requestBody = {
      'wallet': wallet,
      ...
      'surchargeamount': widget.surchargeamount.toString(),
    };
    try {
      final encoder = JsonEncoder.withIndent('  ');
    ...
    print(dashSeparator);
    print("Token as single-line JSON string:");
    print(tokenString);
    print("$separator\n");

    // ============================================
    // REAL API CODE - Wallet Payment API Call
    // ============================================
    try {
      ...
      // Fetch tenant info and lease data
      ...
      try {
        ...
        final tenantResponse = await http.get(
          Uri.parse('$Api_url/api/tenant/get-tenant/${widget.tenantId}'),
          headers: {
        ...
        if (tenantResponse.statusCode == 200) {
          final tenantJson = jsonDecode(tenantResponse.body);
          tenantData = tenantJson['data'] as Map<String, dynamic>?;
        }

        // Fetch lease data for rentalAddress and processor_id
        ...
        print('Error fetching tenant/lease data: $e');
        // Continue with available data
      }

      // Prepare request body according to API specification
      // Note: Backend expects paymentDetails wrapper (like payment_service.dart)
      final paymentDetails = {
        'admin_id': adminId,
        'lease_id': widget.leaseId,
        'tenant_firstName': tenantData?['tenant_firstName']?.toString() ?? '',
        'tenant_lastName': tenantData?['tenant_lastName']?.toString() ?? '',
        'tenant_email': tenantData?['tenant_email']?.toString() ?? '',
        'tenant_id': widget.tenantId,
        'payment_token':
            jsonEncode(paymentData), // Encrypted token as JSON string
      ...
      // Wrap in paymentDetails object (like payment_service.dart does)
      final requestBody = {
        'paymentDetails': paymentDetails,
      };

      print('Wallet Payment API Request: ${jsonEncode(requestBody)}');
      ...
      print('Wallet Payment API Response Status: ${response.statusCode}');
      print('Wallet Payment API Response Body: ${response.body}');
