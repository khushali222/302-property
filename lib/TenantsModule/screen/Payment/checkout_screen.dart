...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _fetchNmiKeys() async {
    ...
    try {
      ...
      // API response structure: { "data": { "merchant_id": "...", ... } }
  ...
  //
  // IMPORTANT: Your Flutter app does NOT generate the token.
  //            The token is generated by Google Pay servers.
  //            Your app only receives and forwards it to your API.
  // ============================================================================
  void onGooglePayResult(Map<String, dynamic> paymentResult) async {
    // The result contains the encrypted token (PaymentData JSON)
    // This token was generated by Google Pay, not by your Flutter app!
    ...
    try {
      final response = await sendPaymentTokenToServer(
        ...
        amount: widget.amount.toStringAsFixed(2), // Use dynamic amount
      );
  ...
  // HOW TOKEN GENERATION WORKS:
  // ============================================================================
  // 1. User clicks Apple Pay button → Apple Pay SDK opens payment sheet
  // 2. User selects card and authorizes (Face ID/Touch ID/PIN)
  // 3. Apple Pay servers GENERATE an encrypted token (NOT your app!)
  // 4. Apple Pay returns the token to this callback function
  // 5. Your app receives the token and sends it to your backend API
  // 6. Your backend API decrypts the token and processes the payment
  //
  // IMPORTANT: Your Flutter app does NOT generate the token.
  //            The token is generated by Apple Pay servers.
  //            Your app only receives and forwards it to your API.
  // ============================================================================
  void onApplePayResult(Map<String, dynamic> paymentResult) async {
    // The result contains the encrypted token (PaymentData JSON)
    // This token was generated by Apple Pay, not by your Flutter app!
  ...
  Future<PaymentResponse> sendPaymentTokenToServer({
    ...
    try {
      ...
      // Get tenant data from SharedPreferences (faster, no API call needed)
      ...
      // Fetch lease data (needed for processor_id and rentalAddress)
      ...
      try {
        ...
        // Continue without lease data
      }

      // Extract inner token for NMI (NMI expects just the token string, not full Google Pay response)
      // For Google Pay: Extract from paymentMethodData.tokenizationData.token
      // For Apple Pay: Use the token directly (it's already in the right format)
        ...
        try {
          ...
          if (pmData?.containsKey('tokenizationData') == true) {
            ...
            if (tokenData?.containsKey('token') == true) {
              final innerToken = tokenData!['token'] as String?;
              if (innerToken != null) {
                // Parse the inner token string to a RAW OBJECT (NOT stringified)
                // Google Pay gives us a JSON string, but NMI needs the raw object
                // Backend will stringify it correctly without corruption/escaping
                try {
                  final rawTokenObject =
                      jsonDecode(innerToken) as Map<String, dynamic>;
                  paymentTokenForNmi =
                      rawTokenObject; // Send as RAW OBJECT, not string
                  print(
                      '[PAYMENT] ✅ Extracted and parsed inner token to RAW OBJECT');
                  print(
                      '[PAYMENT] ✅ Token keys: ${rawTokenObject.keys.toList()}');
                  print(
                      '[PAYMENT] ✅ Token is sent as RAW OBJECT (not stringified, not escaped)');
                  print(
                      '[PAYMENT] ✅ Backend will receive raw object and stringify correctly for NMI');
                  print(
                      '[PAYMENT] ✅ This prevents corruption/escaping/unicode issues');
                } catch (e) {
                  // If parsing fails, something is wrong with the token
                  print('[PAYMENT] ❌ ERROR: Could not parse inner token: $e');
                  print(
                      '[PAYMENT] ❌ Token might be corrupted. Using as string (fallback)');
          ...
          // Fallback to full token if extraction fails
        }
      }
      ...
      final paymentDetails = {
        ...
        'payment_token':
            paymentTokenForNmi, // Send inner token for Google Pay, full token for Apple Pay
        'payment_method': wallet, // "googlepay" or "applepay"
        'amount': amount,
      ...
      print('\n[PAYMENT] ============================================');
      print(
          '[PAYMENT] Sending request to: $Api_url/api/nmipayment/wallet-payment');
      print('[PAYMENT] Amount: \$$amount | Method: $wallet');
      print('[PAYMENT] Payment token type: ${paymentTokenForNmi.runtimeType}');

      // ============================================
      // DETAILED TOKEN LOGGING FOR NMI DEBUGGING
      // ============================================
      print('\n[PAYMENT] 📦 TOKEN BEING SENT TO BACKEND:');
      print('[PAYMENT] ──────────────────────────────────────────────');

      if (paymentTokenForNmi is String) {
        print('[PAYMENT] Token Format: STRING');
        print(
            '[PAYMENT] Token Length: ${paymentTokenForNmi.length} characters');
        print('[PAYMENT] Token Value (COMPLETE):');
        print('────────────────────────────────────────────────────────');
        print(paymentTokenForNmi);
        print('────────────────────────────────────────────────────────');

        // Parse and show structure
        try {
          final parsed = jsonDecode(paymentTokenForNmi) as Map<String, dynamic>;
          print('\n[PAYMENT] Token Structure (when parsed as JSON):');
          print('  ✅ Keys: ${parsed.keys.toList()}');
          print('  ✅ Has signature: ${parsed.containsKey('signature')}');
          print(
              '  ✅ Has protocolVersion: ${parsed.containsKey('protocolVersion')}');
          print(
              '  ✅ Has signedMessage: ${parsed.containsKey('signedMessage')}');
          if (parsed.containsKey('protocolVersion')) {
            print('  ✅ Protocol Version: ${parsed['protocolVersion']}');
          }
          if (parsed.containsKey('signature')) {
            final sig = parsed['signature'] as String?;
            print('  ✅ Signature length: ${sig?.length ?? 0} chars');
          }
          if (parsed.containsKey('signedMessage')) {
            final msg = parsed['signedMessage'] as String?;
            print('  ✅ SignedMessage length: ${msg?.length ?? 0} chars');
          }
        } catch (e) {
          print('[PAYMENT] ⚠️ Token is not valid JSON: $e');
        }
      } else {
        print('[PAYMENT] Token Format: OBJECT');
        final tokenJson = jsonEncode(paymentTokenForNmi);
        print(
            '[PAYMENT] Token Length (when stringified): ${tokenJson.length} characters');
        print(
            '[PAYMENT] Token Keys: ${(paymentTokenForNmi as Map).keys.toList()}');
        print('[PAYMENT] Token Value (COMPLETE - stringified):');
        print('────────────────────────────────────────────────────────');
        print(tokenJson);
        print('────────────────────────────────────────────────────────');
      }

      // Show complete request body
      print('\n[PAYMENT] 📋 COMPLETE REQUEST BODY (JSON):');
      print('[PAYMENT] ──────────────────────────────────────────────');
      try {
        final requestBodyJson = jsonEncode(requestBody);
        print(requestBodyJson);

        // Verify payment_token is sent as JSON string
        print('\n[PAYMENT] ✅ VERIFICATION - Token Format:');
        if (paymentTokenForNmi is String) {
          print('[PAYMENT] ✅ payment_token is sent as JSON STRING format');
          print(
              '[PAYMENT] ✅ In request body: "payment_token": "${paymentTokenForNmi.substring(0, paymentTokenForNmi.length > 50 ? 50 : paymentTokenForNmi.length)}..."');
          print('[PAYMENT] ✅ This is the correct JSON string format for NMI');
        } else {
          print(
              '[PAYMENT] ⚠️ payment_token is sent as OBJECT (will be stringified)');
        }
      } catch (e) {
        print('[PAYMENT] Error encoding request body: $e');
      }

      print('\n[PAYMENT] 🔍 HOW TO CHECK WHAT NMI ACCEPTS:');
      print('[PAYMENT] ──────────────────────────────────────────────');
      print('[PAYMENT] 1. Check NMI Documentation:');
      print('[PAYMENT]    - Search: "NMI Google Pay payment_token format"');
      print('[PAYMENT]    - Check NMI API docs for wallet payments');
      print('[PAYMENT] 2. Check Backend Logs:');
      print('[PAYMENT]    - What does sendNmiRequest receive?');
      print('[PAYMENT]    - What exact format is sent to NMI?');
      print('[PAYMENT] 3. Check NMI Gateway Config:');
      print('[PAYMENT]    - Is gateway in TEST mode?');
      print('[PAYMENT]    - Is merchant ID correct for TEST?');
      print('[PAYMENT]    - Is gatewayMerchantId matching?');
      print('[PAYMENT] 4. Check NMI Error Logs:');
      print('[PAYMENT]    - REFID:626830500 - what does this mean?');
      print('[PAYMENT]    - Check NMI dashboard for detailed error');
      print('[PAYMENT] ──────────────────────────────────────────────');
      print('[PAYMENT] ============================================');
      ...
      final apiElapsed = DateTime.now().difference(apiStartTime);
      print(
          '[PAYMENT] ✅ Response received in ${apiElapsed.inMilliseconds}ms | Status: ${response.statusCode}');

      // Parse and log key response fields
      try {
        final responseJson = jsonDecode(response.body);
        print(
            '[PAYMENT] Response - statusCode: ${responseJson['statusCode']} | message: ${responseJson['message'] ?? 'N/A'}');
        if (responseJson['data'] != null) {
          print(
              '[PAYMENT] Transaction ID: ${responseJson['data']?['transactionid'] ?? 'N/A'}');
          print(
              '[PAYMENT] Response Text: ${responseJson['data']?['responsetext'] ?? 'N/A'}');
        }
      } catch (e) {
        print('[PAYMENT] ⚠️ Error parsing response: $e');
        print('[PAYMENT] Raw response: ${response.body}');
      }
